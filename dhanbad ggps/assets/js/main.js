...
(function ($) {
    ...
    $(document).ready(function () {
       ...
       function mobileMenu() {
        ...
        // Add dropdown indicators and toggle submenu
...
document.addEventListener("DOMContentLoaded", function () {
    const currentPage = window.location.pathname.split("/").pop(); // e.g., "about-us.html"

    // Select all menu links in the main nav
    const navLinks = document.querySelectorAll(".main-menu a");

    navLinks.forEach(link => {
        const href = link.getAttribute("href");

        // Ignore anchor links, JS links, or external URLs (unless needed)
        if (!href || href.startsWith("http") || href.startsWith("#") || href === "#") return;

        // Only compare file name (last part of URL)
        ...
        if (linkPage === currentPage) {
            ...
            // Highlight parent <li> if it's in a dropdown
            const parentLi = link.closest("li");
            if (parentLi) {
                parentLi.classList.add("active");

                // Also highlight the main parent <li> in case it's a submenu
                const grandParentLi = parentLi.closest("ul")?.closest("li");
                if (grandParentLi) {
                    const parentAnchor = grandParentLi.querySelector(":scope > a");
